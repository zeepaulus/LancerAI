...
class JobMatchResult(Base):
    ...
    job_id: Mapped[str | None] = mapped_column(String(36), ForeignKey("job_listings.id"), nullable=True, index=True)

    match_score: Mapped[float] = mapped_column(Float, default=0.0)
    frequency_score: Mapped[float | None] = mapped_column(Float, nullable=True)
    position_score: Mapped[float | None] = mapped_column(Float, nullable=True)
